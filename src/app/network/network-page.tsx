...
export default function NetworkPage({ session }: { session: Session }) {
  ...
  return (
    <div className="os-reveal" style={{ display: "flex", flexDirection: "column", minHeight: "100vh" }}>
      <div className="os-page-header">
        <div>
          <div className="os-page-title">Comunidad</div>
        ...
        {activeTab === "decisions" && <button onClick={() => setShowDecForm(true)} className="btn-primary">+ Registrar decisión</button>}
      </div>

      {/* Tab bar */}
      <div style={{ padding: "0 2.5rem", display: "flex", gap: "0.25rem", borderBottom: "1px solid var(--border)" }}>
        {([
          { id: "feed", label: "Feed de la comunidad", count: threads.length },
        ...
        {activeTab === "feed" && (
          <>
            ...
            <div style={{ width: 200, flexShrink: 0, display: "flex", flexDirection: "column", gap: "0.25rem" }}>
              ...
              <button onClick={() => setFilter("all")} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0.6rem 0.75rem", borderRadius: "0.5rem", fontSize: "0.85rem", fontWeight: filter === "all" ? 700 : 500, color: filter === "all" ? "var(--ink)" : "var(--ink-3)", background: filter === "all" ? "var(--surface-2)" : "transparent", border: "none", cursor: "pointer", textAlign: "left" }}>
                <span>Todos</span>
                <span style={{ fontSize: "0.75rem", color: "var(--ink-3)" }}>{threads.length}</span>
              </button>
              {CATEGORIES.map((c) => (
                <button key={c.value} onClick={() => setFilter(filter === c.value ? "all" : c.value)} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0.6rem 0.75rem", borderRadius: "0.5rem", fontSize: "0.85rem", fontWeight: filter === c.value ? 700 : 500, color: filter === c.value ? "var(--ink)" : "var(--ink-3)", background: filter === c.value ? "var(--surface-2)" : "transparent", border: "none", cursor: "pointer", textAlign: "left" }}>
